...
if grep -R -n -E '^(calamares|calamares-data|calamares-settings-debian|subiquity|curtin|casper|gnome-core|gnome-shell|gdm3|xfce4|lightdm)$' "$profile/config/package-lists"; then
  echo 'erro: instalador ou desktop alternativo presente na lista' >&2
  exit 1
fi

grep -qx kde-full "$profile/config/package-lists/playos-kde-full.list.chroot"
grep -qx sddm "$profile/config/package-lists/playos-kde-full.list.chroot"

echo 'preflight estático KDE concluído'
