...
if [ -d "$BACKGROUNDS_DIR" ] && [ -f "$ASSETS_DIR/playos-wallpaper.png" ]; then
    echo ">> [Wallpaper] Sobrescrevendo todos os planos de fundo Ubuntu/Raccoon..."
    mkdir -p "$BACKGROUNDS_DIR/playos"
    cp "$ASSETS_DIR/playos-wallpaper.png" "$BACKGROUNDS_DIR/playos/default.png"

    # Sobrescrever arquivos específicos identificados na auditoria
    # -name "*ubuntu*" -o -name "*raccoon*" -o -name "warty*"
    find "$BACKGROUNDS_DIR" -type f \( -name "*ubuntu*" -o -name "*raccoon*" -o -name "warty*" \) -exec cp "$ASSETS_DIR/playos-wallpaper.png" {} \;

    # Garantir o link simbólico histórico
    rm -f "$BACKGROUNDS_DIR/warty-final-ubuntu.png"
    cp "$ASSETS_DIR/playos-wallpaper.png" "$BACKGROUNDS_DIR/warty-final-ubuntu.png"
fi
