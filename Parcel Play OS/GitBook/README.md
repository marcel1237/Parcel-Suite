...
por Git Sync. Ela não publica um site sozinha e não contém credenciais, tokens
ou configuração de uma organização GitBook.
...
de verdade continua na raiz do projeto, em `supervised_learning/`, nos catálogos
e nos artefatos de `build/`.

Estados técnicos seguem o vocabulário do projeto: `fact`, `inference`, `decision`
, `proposal`, `implementation`, `result` e `unknown`.
...
gitbook whoami
```
Não execute comandos de autenticação sem uma decisão explícita de conectar este
checkout a uma conta GitBook.
